...
def initialize_logger() -> Tuple[logging.Logger, str]:
    ...
    log_file = os.path.join(log_dir, f"layer2_log_{timestamp}.log")
...
def generate_report(results: List[TestResult], logger: logging.Logger) -> None:
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    pdf_file = os.path.join(report_dir, f"layer2_report_{timestamp}.pdf")
    csv_file = os.path.join(report_dir, f"layer2_report_{timestamp}.csv")
...
def write_pdf_report(results: List[TestResult], pdf_path: str) -> None:
    try:
        ...
        pdf.cell(40, 10, "Layers - OSI Testing Report: Layer 2")
        pdf.ln(12)
...
def generate_full_report(results: List[TestResult], full_output: str) -> str:
    ...
    os.makedirs(report_dir, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    report_path = os.path.join(report_dir, f"layer2_raw_output_{timestamp}.txt")
